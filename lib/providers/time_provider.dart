...
String _formatDuration(Duration d) {
  return (d.isNegative ? '- ' : '+ ') +
      d.toString().split('.').first.split('-').last.padLeft(8, "0");
}
...
final deltaProvider = Provider<String>((ref) {
  ...
  final lastPassed = ref
      .watch(currentTrackProvider)
      ?.points[ref.watch(trackIndexProvider)];
...
final elapsedProvider = Provider<String>((ref) {
  ...
  return _formatDuration(
    DateTime.now().difference(
      ref.watch(checkpointProvider).firstOrNull?.time ?? DateTime.now(),
    ),
  );
});
final remainingProvider = Provider<String>((ref) {
  ...
  final lastPassed = ref
      ...
      ?.points[ref.watch(trackIndexProvider)];
  return _formatDuration(
    (lastPassed?.time ?? DateTime.now()).difference(
      ref.read(currentTrackProvider)?.points.last.time ?? DateTime.now(),
    ),
  );
});
